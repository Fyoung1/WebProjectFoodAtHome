...
@section('content')
    <div class="MilkProductContainer">
        <h1 style="position: relative;top:-100px" >Колбасы и сосиски</h1>
        <div class="MilkProduct">
            @foreach ($SausagesProducts as $SausagesProduct)
                ...
                <img style="width: 125px;height: 125px;margin-top: 5%;"  src="{{ url($SausagesProduct->image) }}" alt="" class="">
                <h2 style="position: relative;margin-top:19%;right: 10%;width:10px" >{{ $SausagesProduct->price }}₽</h2>
                <p style="position: relative;width: 1%;margin-top:22%;right: 11%;display: flex;">{{ $SausagesProduct->name}}  {{$SausagesProduct->description }}</p>
                <input type="hidden" value="1" name="quantity">
                <button style="position: relative; height:10px; top:335px;left: -11%;border: 0px solid #FFFFFF;color:#1d2124;font-size: 30px;" id="{{$SausagesProduct->id}}" onclick='myFunction(this.id)'>+</button>
            @endforeach
