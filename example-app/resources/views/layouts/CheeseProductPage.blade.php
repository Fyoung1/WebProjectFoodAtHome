...
@section('content')
    <div class="MilkProductContainer">
        <h1 style="position: relative;top:-100px" >Сыры</h1>
        <div class="MilkProduct">
            @foreach ($CheeseProducts as $CheeseProduct)
                ...
                <img style="width: 125px;height: 125px;margin-top: 5%;" src="{{ url($CheeseProduct->image) }}" alt="" class="">
                <h2 style="position: relative;margin-top:19%;right: 10%;width:10px" >{{ $CheeseProduct->price }}₽</h2>
                <p style="position: relative;width: 1%;margin-top:22%;right: 11%;display: flex;">{{ $CheeseProduct->name}}  {{$CheeseProduct->description }}</p>
                <input type="hidden" value="1" name="quantity">
                <button style="position: relative; height:10px; top:315px;left: -11%;border: 0px solid #FFFFFF;color:#1d2124;font-size: 30px;" id="{{$CheeseProduct->id}}" onclick='myFunction(this.id)'>+</button>
            @endforeach
