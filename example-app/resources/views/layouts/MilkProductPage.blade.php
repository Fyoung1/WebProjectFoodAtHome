...
@section('content')
    <div class="MilkProductContainer">
        <h1 style="position: relative;top:-100px" >Молоко и яйца</h1>
        <div class="MilkProduct" style="position: relative;margin-top: -50px;">
        @foreach ($MilkProducts as $MilkProduct)
            ...
            <img style="width: 125px;height: 125px;margin-top: 5%;" src="{{ url($MilkProduct->image) }}" alt="" class="">
            <h2 style="position: relative;margin-top:19%;right: 10%;width:10px" >{{ $MilkProduct->price }}₽</h2>
            <p style="position: relative;width: 1%;margin-top:22%;right: 11%;display: flex;">{{ $MilkProduct->name}}  {{$MilkProduct->description }}</p>
            <input type="hidden" value="1" name="quantity">
            <button style="position: relative; height:10px; top:325px;left: -10%;border: 0px solid #FFFFFF;color:#1d2124;font-size: 30px;" id="{{$MilkProduct->id}}" onclick='myFunction(this.id)'>+</button>
        @endforeach
        </div>
    </div>
@endsection
