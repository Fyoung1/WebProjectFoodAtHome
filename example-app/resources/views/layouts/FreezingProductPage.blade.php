...
@section('content')
    <div class="MilkProductContainer">
        <h1 style="position: relative;top:-100px" >Заморозка</h1>
        <div class="MilkProduct">
            @foreach ($FreezingProducts as $FreezingProduct)
                ...
                <img style="width: 125px;height: 125px;margin-top: 5%;" src="{{ url($FreezingProduct->image) }}" alt="" class="">
                <h2 style="position: relative;margin-top:19%;right: 10%;width:10px" >{{ $FreezingProduct->price }}₽</h2>
                <p style="position: relative;width: 1%;margin-top:22%;right: 11%;display: flex;">{{ $FreezingProduct->name}}  {{$FreezingProduct->description }}</p>
                <input type="hidden" value="1" name="quantity">
                <button style="position: relative; height:10px; top:335px;left: -11%;border: 0px solid #FFFFFF;color:#1d2124;font-size: 30px;" id="{{$FreezingProduct->id}}" onclick='myFunction(this.id)'>+</button>
            @endforeach
